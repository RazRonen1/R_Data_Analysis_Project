...
author: Raz Ronen & Yuval Rubinshtein
output:
...
<h1>The Data:</h1>
* We used the Israeli chaser data from kaggle  ([link](https://www.kaggle.com/datasets/thomaskonstantin/the-chasegame-show-israel-episode-data))

* Each row in the raw data describes a specific episode, there are a total of 100 episodes (within 3 seasons)

* When we use the term **'Fast Round'** - it's the first round for each individual player
* When we use the term **'Final Round'** - it's the last round where all players are playing together as a team



<h1>Goals:</h1>
* Our main goal is to track after the pattern of players who won the chaser, we will try to examine how the group's age distribution and gender distribution affect the winning ratio
...
Second, load our data from the csv file:
...
* The dataset is built in a horizental structure in which each row represents the entire team (and a specific episode).
...
* Now we have **404 rows** instead of 101 (because there are 4 players in each team)
...
* We are going to add few columns:
  ...
  + **risk_bonus** - int type, for how much money did the player took the risk? (0 if not, negative if chose the safer way).
...
<h4>We are going to check the different distributions of our variables:</h4>

* Lets start with the right answers distribution:

```{r}
questions_dist <- ggplot(chaser, aes(x=Team_question_answered)) +
  geom_histogram(bins=10, fill="tan1") + labs(title="Right Answers Distribution", xlab="Team's right answers") + theme(plot.title = element_text(hjust = 0.5))
...
* We'll try to understand better if our variables distributions are normal using qq plots, if it is - we expect to see a clear straight line:
...
* We can see a few interesting things regard the players who chose to **take a smaller amount** in order to be one step ahead of the chaser (more chances to win the round):
  + There were only 10 players who chose to be safer out of 404 total (~2.5%)
...
* We can also see that in a 95% confidence level:
  + The age affects the rate in which **Male** players will tend to take risks.
  + But it does **not** affect the **Female** players.
  


<h5>Lets visualize the risk taking proportions by age & gender:</h5>

(**Note-** there is the exact same amount of male and female players).
...
graph_by_age <- chaser %>%
  ...
  geom_bar(stat = "identity", position=position_fill()) +
    labs(title="Risk Taking Proportions by Age", x = "Age Group")
...
```
  ...
  + If we look at all the players in general, we can see that there is an increment in the risk taking proportions as the age increases.
...
* (**Note- **The fast round groups are the amount gained in the fast round divided to different ranges)
...
* We can see a **negative correlation** between the average age and the age deviation to the number of questions answered in the final round.
* Though the R-squared is relatively low, the model is **statistically significant** since the p-value is very low.
...
* In general, younger and homogenize groups are more likely to perform better in the last round (meaning- more likely to win).
...
* We found some interesting things about the chaser game show:
  ...
  * The average age of the group and it’s deviation has a negative effect on the winning rate (negatively affects the questions answered in the last round, and thus affects the winning rate). The younger the average age is and the more homogeneous it is - the higher are the chances to win.
...
* While working on the project we achieved experience in - code writing, data visualizing, statistics tests & principles implementations. 
